...
def generate_htmlpage(start_time, end_time, category, release, version, tabledata=[]):
    ...
    page.h2('Report for NetDiagnostics Core: ' + category, _class='lead')
    page.td.close() 
    ...
    page.a(
        ...
        href='http://10.10.30.37/reports/graphs/NetDiagnostics/%s/%s/B%s' % (
            category.lower(), 
            release, 
            version
        ),
...
def parse_results_file(f):
    with open(f) as rfile: 
        for line in rfile:
            appication, name, testrun, categoryid, componentid, status, desc = line.strip().split(',')
            hash_data = dict(
