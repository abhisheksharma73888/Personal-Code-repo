...
def get_perf_metrics_as_hash(trun)
    ...
    if File.exists? perf_csv_file
        File.open(perf_csv_file).each do|row|
            ...
            perf_metric_hash[metric] = max.to_f
        end
    end
    perf_metric_hash
end
    ...
    <table style='border: 0px solid #FFF !important'>
      <tr style='border: 0px; !important'>
        <td style='width:65%; font-weight: normal; background-color: #FFF; border: 0px !important; vertical-align: text-top'>
          <h2 style='font-size: 1.68rem; color: #616161; font-weight: 300;'>Report for: #{@title}</h2>
        </td>
...
def process(baseline_hash, testrecords)
    ...
    testrecords.each do |test_hash|
        ...
        test_hash.each do|metric, value|
            ...
            avg_c = value
            # If some metric is not present in baseline
            # Todo: Fix this. Flowpaths will not be present
            if avg_b.nil?
                avg_b = 0.0
            end
            tolerance = (avg_b * (@tolerancepct/100))
            if avg_c.is_a?(Float) && ((avg_c > (avg_b + tolerance))||(avg_c < (avg_b - tolerance))) 
...
if __FILE__ == $0
    ...
    unless @title
	@title = "NetDiagnostics Enterprise Performance"
    end
    puts main
end
