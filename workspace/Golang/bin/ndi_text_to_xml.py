...
class TestResultsModel(object):
    def __init__(self, id, testrun, component, category, status, description):
        self.id          = id
...
def processTestResultsFile(testresultFile):
    ...
    with open(testresultFile, 'r') as f:
        for line in f:
            ...
            id, testrun, component, category, status, desc = line.split(",")
            l.append(TestResultsModel(id, testrun, component, category, status, desc.strip()).asDict())
